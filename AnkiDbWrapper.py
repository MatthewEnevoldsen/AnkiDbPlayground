...
class AnkiDbHelper:
    ...
    def tagSenRatios(self, known : str, unknown: str):
        pass
        # jap = 0
        # ratios =  self.getSenRatios()
        # for n in self.getNotesOfType("AllSentences"):
    ...
    def getAllSentences(self):
        ...
        sen = set()
        for n in self.getNotesOfType("AllSentences"):
            sen.add(self.ankiStrings.stripNoise(self.getField(n.flds, jap)))
        return list(sen)

    def getAllVocabSet(self) -> Set[str]:
        allVocab = set()
        for n in self.getNotesOfType('Core10K'):
            flds = self.getFields(n.flds)
            vocab = self.get10KVocab(flds)
            allVocab.add(vocab)
        return set(allVocab)

    def getKnownVocabSet(self) -> Set[str]:
        vocabFld = 1
        known = set()
        for n in self.getNotesOfType('Core10K'):
            flds = self.getFields(n.flds)
            vocab = self.ankiStrings.toBaseVocab(flds[vocabFld])
            cards = set(self.session.query(self.cards).filter(self.cards.nid == n.id).all())
    ...
    def updateField(self, note, fldId: int, value: str):
        ...
        flds[fldId] = value
        note.fields = self.toFields(flds)
